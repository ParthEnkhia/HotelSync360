...
jest.mock("./StaffManagement", () => () => <div>Staff Management</div>);
jest.mock("./ReferenceSetup", () => () => <div>Property Setup</div>);
...
test("renders a clean login screen first", async () => {
  render(<App />);

  expect(await screen.findByText(/HotelSync360/i)).toBeInTheDocument();
