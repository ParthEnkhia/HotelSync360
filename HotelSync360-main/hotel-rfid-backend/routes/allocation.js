...
router.post("/create", async (req, res) => {
  try {
    ...
    const [staffRows] = await pool.query(
      "SELECT property_id, is_active, status FROM STAFF WHERE staff_id = ? LIMIT 1",
      [staff_id]
    );
    if (!staffRows.length) {
      return res.status(404).json({ error: "Staff member not found" });
    }
    if (!staffRows[0].is_active || staffRows[0].status !== "ACTIVE") {
      return res.status(400).json({ error: "Staff member must be active" });
    }
    ...
    if (allocated_by_staff_id) {
      ...
      if (!allocatedByRows[0].is_active || allocatedByRows[0].status !== "ACTIVE") {
        return res.status(400).json({ error: "Allocated-by staff member must be active" });
      }
