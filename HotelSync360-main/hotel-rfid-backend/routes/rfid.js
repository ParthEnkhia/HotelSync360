  ...
  try {
    ...
    if (assignee_type && assignee_id) {
      ...
      const activeCondition =
        assignee_type === "GUEST"
          ? "AND is_active = TRUE AND status = 'CHECKED_IN'"
          : "AND is_active = TRUE AND status = 'ACTIVE'";
      const [entityRows] = await connection.query(
        `SELECT ${entityColumn} FROM ${entityTable} WHERE ${entityColumn} = ? ${activeCondition} LIMIT 1`,
        [assignee_id]
      );

      if (!entityRows.length) {
        await connection.rollback();
        return res.status(404).json({ error: `${assignee_type} not found or inactive` });
      }
